...
class GenericAgent(MyBaseAgent):
    ...
    def act(self, obs, action_space, info):
        ...
        if my_kick:
            ...
            kickable, _ = self._kickable_positions(obs, info["moving_direction"])
        ...
        if all([is_survivable[constants.Action.Bomb],
                best_action in [constants.Action.Stop, constants.Action.Bomb]]):
        ...
        if best_action not in [None, constants.Action.Bomb]:
            ...
            # if I see one enemy and a teammate, just focus on killing the enemy
            if any([total_frac_blocked[next_position] > 0.1,
                    len(enemy_positions) == 1 and teammate_position is not None]):
                if utility.position_is_flames(board, next_position):
        ...
        #
        # Move towards the last seen enemy
        #
