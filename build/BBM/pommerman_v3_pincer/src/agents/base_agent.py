...
class MyBaseAgent(BaseAgent):
    ...
    def _board_sequence(self, board, bombs, flames, length, my_position,
                        ...
                        enemy_bomb=0, enemy_blast_strength=dict()):
        """
        ...
        enemy_blast_strength_map = np.full(_board.shape, 2)
        for enemy_position in enemy_positions:
            enemy = _board[enemy_position]
            enemy_blast_strength_map[enemy_position] = enemy_blast_strength.get(enemy, 2)
        ...
        for t in range(length):
            ...
            if t == 0 and enemy_mobility > 0:
                ...
                for enemy in enemies:
                    ...
                    kickable, _ = self._kickable_positions(_obs, moving_direction,
            ...
            #if __bombs:
            #    _bombs = list(set(_bombs + __bombs))
            #    for b in _bombs:
            #        _board[b.position] = constants.Item.Bomb.value
            ...
            if __flames:
                #for f in set(__flames) - set(_flames):
                #    print("extra flames", f.position, f._life)
                positions = list()
                life = np.zeros(_board.shape)
                for f in set(_flames + __flames):
                    position = f.position
                    positions.append(position)
                    life[position] = max([life[position], f._life])
                _flames = list()
                ...
                for x, y in enemy_positions:
                    ...
                    for dx, dy in [(1, 0), (-1, 0), (0, 1), (0, -1)]:
                        ...
                        if any([utility.position_is_passage(_board, next_position),
                            ...
                            enemy_blast_strength_map[next_position] \
                                = max([enemy_blast_strength_map[next_position],
                                       enemy_blast_strength_map[(x, y)]])
    ...
    def _get_survivable_actions(self, survivable, obs, curr_bombs, curr_flames,
                                enemy_mobility=0, enemy_bomb=0, enemy_blast_strength=dict()):
        ...
        if all([obs["ammo"] > 0, obs["bomb_life"][my_position] == 0]):
            ...
            list_boards_with_bomb, _ \
                = self._board_sequence(board_with_bomb,
                                       ...
                                       enemy_blast_strength=enemy_blast_strength)
            ...
            #print("boards with bomb")
            #for t, b in enumerate(list_boards_with_bomb):
            #    print(t)
            #    print(b)
            #    if t > 2:
            #        break
        ...
        for next_position in on_board_next_positions:
            ...
            for dx, dy in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
                ...
                if all([moving_direction[coming_position] == constants.Action.Down,
                        ...
                        dy == 0]):
                    # coming from above
                    #print("can kick a moving bomb coming from", coming_position, "above to", next_position)
                    kickable.add(next_position)
                    break
    ...
    def _get_most_survivable_action(cls, n_survivable):
